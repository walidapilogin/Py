...
def send_spam(player_id):
    ...
    url = f"https://lovely-moral-asp.ngrok-free.app/api/spam_gherab?id={player_id}"
    response = requests.get(url)
...
def telegram(message):
    token = '8147721701:AAF_yd-LqYfDxIjG_tPJOn8J5xz9g1bZUU8'
    chat_id = '-1002246313542'
...
def Fake_Friend(id, client_socket):
    url = f"https://api-delta-two.vercel.app/fake"
...
def dance(id, dance_nbr=None):
    Ghost_Emote = [
        f"050000002008{id}100520162a1408{id}1084fbb8b1032a0608{id}",
    ...
    try:
        if isinstance(dance_nbr, str) and dance_nbr.isdigit():
            dance_nbr = int(dance_nbr)
        elif not isinstance(dance_nbr, int):
            dance_nbr = None
    except ValueError:
        dance_nbr = None
    
    if dance_nbr is not None and 1 <= dance_nbr <= len(Ghost_Emote):
        return Ghost_Emote[dance_nbr - 1]
    else:
        return random.choice(Ghost_Emote)
...
class Proxy:
            ...
            if remote in r:
                ...
                if '1200' in data.hex()[0:4] and b'GroupID' in data:
                    ...
                    if '1200' in data.hex()[:4]:
                        ...
                        if current_time - self.last_check_time >= 86400:  # 86400 &#1579;&#1575;&#1606;&#1610;&#1577; = 24 &#1587;&#1575;&#1593;&#1577;
                ...
                if '1200' in data.hex()[0:4] and b"/help" in data:
                    ...
                    message = (
    f"[882afa][c][b]-----------------------------------\n\n"
    f"[{generate_random_color()}][c][b]الاوامر المتاحة في البوت هي : \n\n"
    f"[2afa35][c][b]معلومات الاعب   : \n"
    f"[42e6f5][b]\n"
    f"/info12345678\n\n"
    f"[2afa35][c][b] الصديق الوهمي  : \n"
    f"[42e6f5][b]\n"
    f"/id12345678\n"
    f"[2afa35][b]\n"
    f"سبام الدعوات للمجموعة عن طريق الأيدي\n\n"
    f"[42e6f5][c][b]  --12345678\n\n"
    f"[2afa35][c][b]لتحويل السكواد ل 5 اشخاص   :\n\n"
    f"[42e6f5][c][b]/5\n\n"
    f"[2afa35][c][b] للتحويل سكواد من 6 اشخاص    : \n\n"
    f"[42e6f5][c][b]/6\n\n"
    f"[2afa35][c][b]لتفعيل الرقصات : [{generate_random_color()}][c][b]\n\n"
    f"[{generate_random_color()}][c][b]/dance\n"
    f"[{generate_random_color()}][b]\n"
    f"/dance1     /dance2     -->  11 \n\n"
    f"[2afa35][c][b]لعمل سبام دعوات الانضمام   : \n"
    f"[42e6f5][c][b]\n"
    f"ON              /inv\n\n"
    f"OFF           /-inv\n\n"
    f"[882afa][c][b]-----------------------------------\n\n"
    f"[ffff00][c][b] instgram :  [fab12a][c][b] zodiag_hex \n\n"
    ...
    f"[882afa][c][b]-----------------------------------"
                ...
                if '1200' in data.hex()[0:4] and b'--' in data:
                	i = re.split('--', str(data))[1]
                	id = str(i).split('(\\x')[0].strip()
                	if '***' in id:
                		id = id.replace('***', '106')
                	message = (
                	f"[ff7f50][c][b]-----------------------------------\n\n"
                	f"[{generate_random_color()}][c][b]جاري ارسال سبام للاعب : \n\n"
                	f"[ffffff][c][b]{id}"
                	f"\n\n[ff7f50][c][b]-----------------------------------"
                	)
                	send_spam(id)
                	threading.Thread(target=ResponseMsg,args=(message,client, self.EncryptedPlayerid)).start()
                	message_telegram = f"User_id = {Decrypted_id(self.EncryptedPlayerid)}\nused_command  = attack_squad\ntarget_id = {id}"
                	threading.Thread(target=send_telegram_message,args=(message_telegram,)).start()
    ...
    def fetch_data_from_url(self):
        data_url = "http://example/data.txt"
        try:
